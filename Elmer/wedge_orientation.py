...
def load_inputs() -> tuple[list[dict[str, float]], dict, dict, dict]:
    ...
    return plinth, config, metadata, {
        ...
        "angle": float(config["wedge_angle_from_vertical_deg"]),
        "outer_radius": float(config["wedge_anchor_radius_m"]),
        "inner_radius": float(config["wedge_opposite_vertex_radius_m"]),
    }
...
def main() -> None:
    ...
    inner_radius = inputs["inner_radius"]
    ...
    start = float(metadata["wedge_start_station_m"])
    end = float(metadata["wedge_end_station_m"])
    width = end - start
    ...
    # Plan view: the wall is the outer radius and the wedge toe is the inner radius.
    ...
    # Chainage logic: positive depths below the crest, not signed mesh Z.
    ...
    # Maximum local section: the wedge occupies z=-25 to the bedrock base z=-29.
    ...
    ax3.plot([4, 4 + maximum_thickness], [-29, -29], color="tab:red", linewidth=2.2, label="Wedge bottom to toe, r = 74 m")
    ...
    ax3.text(4.45, -27.0, f"{90.0 - angle:.2f}°", rotation=-(90.0 - angle), color="tab:red", va="center")
    ...
    # Isometric wedge band between r=76 and the variable toe radius.
    ...
    fig.text(0.5, 0.02, f"h = p - 25;  t = h tan({angle:.2f}°);  w = {start:.2f} to {end:.2f} m = {width:.2f} m", ha="center", fontsize=10)
